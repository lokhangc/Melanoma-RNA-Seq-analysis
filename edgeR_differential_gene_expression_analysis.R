...
The number of differentially expressed genes is summarized, including the number of non -
  significant, significantly up regulated and significantly down regulated differentially expressed genes.
```{
  r
}
...
The entire list of differentially expressed genes can be extracted using the topTags() function. FDR is not calculated, significantly changed genes are selected with p -
  value < 0.05.
```{
  r
}
